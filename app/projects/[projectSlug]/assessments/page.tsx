...
export default async function ProjectAssessmentPage({
  ...
  return <ProjectAssessmentPageContent projectSlug={projectSlug} />;
}
...
async function ProjectAssessmentPageContent({
  ...
  const [project, grid, submissions, progressBySubmissionId] =
    await Promise.all([
  ...
  const questions = firstSubmissionId
    ? Object.entries(grid).map(([id, { label }]) => ({
        ...
        href: `/projects/${projectSlug}/assessments/submissions/${firstSubmissionId}/questions/${id}`,
      }))
    : [];
  ...
  return (
    <Container component="main" maxWidth="md" sx={{ py: 5 }}>
      ...
      <Box sx={{ mb: 3 }}>
        <Button href={projectOverviewPath(projectSlug)} variant="outlined">
      ...
      <List component="nav" aria-label="Submission list" sx={{ mb: 3 }}>
        {submissions.map((submission) => {
          ...
          return (
            <ListItemButton
              key={submission.id}
              href={`/projects/${projectSlug}/assessments/submissions/${submission.id}`}
