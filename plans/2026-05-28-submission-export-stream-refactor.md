...
Submission export now streams nested data rows from `src/export/submissionExport.ts`, formats CSV in `src/export/submissionExportCsv.ts`, and parses request options in a route-local helper. CSV headers, column order, and output semantics were preserved.
