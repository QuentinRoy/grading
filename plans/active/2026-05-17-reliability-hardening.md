...
- Tier 1: 1 open, 0 in progress, 6 verified
...
- Remaining scope: R-010 (Tier 1), then R-012, R-013, R-014, R-015 (Tier 2)
...
- 2026-06-22: Code-review follow-up on the R-009 PR (no risk register changes; same Verified status). Extracted the duplicated project/question/rubric/student/submission/assessment fixture setup shared between `src/import/assessmentImportRoundtrip.integration.test.ts` and `src/export/submissionExport.integration.test.ts` into `src/test/mixedRubricAssessmentFixture.ts`. Parallelized independent fixture writes with `Promise.all` (rubric subtype inserts, rubric-assessment subtype inserts) per the project's "avoid successive async work that can run in parallel" convention. Reshaped `createStudentFixture`/`createIndividualSubmissionFixture` into bulk `createStudentFixtures`/`createIndividualSubmissionFixtures` (one multi-row insert per call instead of one round trip per row), typed with a `Promise.all`-style tuple-preserving generic (`FixtureTuple`, with `StudentFixtureTuple`/`SubmissionFixtureTuple` instances) so a literal-id array argument gets back a same-length, per-element-literal-typed result and callers can destructure without `undefined` checks or non-null assertions; added `Writable<T>` to `src/utils/utils.ts` to strip the `readonly` a `const` type parameter puts on picked literal properties. Also brought the new fixture helpers' signatures in line with `docs/guides/typescript-api-design.md` (named-object domain params after a leading `db`). Local verification: `pnpm run check --fix`, `pnpm run check-types`, `pnpm vitest run --project=unit --project=integration src/import/ src/export/ src/utils/` (87 passing).
