...
**Guide vs skill.** The dividing line is *activation*, not audience (agents read guides too). A guide is *navigated to* — a human browses it, or an agent loads it because the routing table pointed there. A skill (`.agents/skills/*`) *auto-activates* when the task matches its description. So a coding convention that is **cleanly domain-scoped** — component styling, user-facing error handling, test fixtures — makes a good skill: it fires exactly when that kind of work happens. A convention that applies to **almost every edit** (general code style; TypeScript API shape in a TS-only repo) is a poor skill — it would trigger always, which is just always-loaded with extra steps — so it stays a short rule in `AGENTS.md`, with depth in a guide and its mechanically-checkable parts moved to lint rules. Reserve `docs/guides/` for those durable explanations and for human/process workflows. (Routing the conditional conventions into skills is tracked in `plans/2026-06-23-agent-instruction-layer-hardening.md`.)
